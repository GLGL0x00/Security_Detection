...
def tracker():
    ...
    while True:
        ...
        result = model.predict(frame, stream=True, max_det=1, vid_stride=5, imgsz=512, stream_buffer=True)

        # Process the frame with YOLO object detection (model2)
        result2 = model2(frame, classes=[0],imgsz=512, max_det=1, vid_stride=5, stream=True, batch=5, stream_buffer=True)
    ...
    with open("criminals.json") as json_file2:
    ...
    with open("criminals.json", "w") as file:
